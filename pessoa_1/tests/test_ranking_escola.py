"""EscolaClassificada / classificar_escola / posicao_na_fila -- ranking por
escola na linha de vulnerabilidade + distancia (ver test_vulnerabilidade.py
...
def test_classificar_escola_ordena_por_score_decrescente():
    escola = Escola(escola_id="E1", localizacao=loc(0.0, 0.0))
    alta = cand("alta", {"bolsa_familia": 1, "deficiencia": 1})   # score = 2
    media = cand("media", {"bolsa_familia": 1})                  # score = 1
    baixa = cand("baixa", {})                                    # score = 0

    classificada = classificar_escola(escola, [media, baixa, alta])

    assert [c.crianca_id for c in classificada.candidatos] == ["alta", "media", "baixa"]
    assert classificada.escola_id == "E1"
...
def test_distancia_entra_no_ranking_junto_com_vulnerabilidade():
    ...
    classificada = classificar_escola(
        escola, [longe_com_vulnerabilidade, perto_sem_vulnerabilidade], alcance_km=5.0
    )
...
def test_empate_e_resolvido_por_sorteio_reproduzivel_com_semente():
    ...
    primeira = classificar_escola(escola, empatados, semente=42)
    segunda = classificar_escola(escola, empatados, semente=42)
...
def test_sementes_diferentes_sorteiam_ordens_diferentes():
    ...
    ordens = {
        tuple(c.crianca_id for c in classificar_escola(escola, empatados, semente=s).candidatos)
        for s in range(10)
    }
...
def test_sorteio_de_empate_nao_atropela_diferenca_de_score():
    ...
    for semente in range(5):
        classificada = classificar_escola(escola, [*empatados, melhor], semente=semente)
        assert classificada.candidatos[0].crianca_id == "melhor"


def test_classificar_escola_sem_semente_nao_falha():
    escola = Escola(escola_id="E1", localizacao=loc())
    classificada = classificar_escola(escola, [cand("a"), cand("b")])
...
def test_posicao_na_fila_encontra_a_posicao_correta():
    escola = Escola(escola_id="E1", localizacao=loc())
    classificada = classificar_escola(
        escola, [cand("c", {}), cand("b", {"x": 1}), cand("a", {"x": 1, "y": 1})]
    )
...
def test_posicao_na_fila_none_quando_crianca_nao_esta_na_lista():
    escola = Escola(escola_id="E1", localizacao=loc())
    classificada = classificar_escola(escola, [cand("a")])
    assert posicao_na_fila("zzz", classificada) is None


def test_classificar_escola_nao_muta_a_lista_de_entrada():
    escola = Escola(escola_id="E1", localizacao=loc())
    candidatos = [cand("b"), cand("a", {"x": 1})]
    antes = list(candidatos)
    classificar_escola(escola, candidatos)
    assert candidatos == antes


def test_classificar_escola_com_lista_vazia():
    escola = Escola(escola_id="E1", localizacao=loc())
...
def test_adicionar_candidato_entra_na_posicao_que_o_score_determina():
    ...
    classificada = classificar_escola(
        escola, [cand("baixo", {}, longe), cand("alto", {"x": 1})]
    )
...
def test_adicionar_candidato_em_classificacao_vazia():
    escola = Escola(escola_id="E1", localizacao=loc())
    vazia = classificar_escola(escola, [])
...
def test_adicionar_candidato_com_semente_e_reproduzivel_em_empate():
    escola = Escola(escola_id="E1", localizacao=loc())
    classificada = classificar_escola(escola, [cand("a"), cand("b")], semente=1)
...
def test_remover_candidato_tira_da_lista():
    ...
    classificada = classificar_escola(
        escola, [cand("a", {"x": 1, "y": 1}), cand("b", {"x": 1}), cand("c", {})]
    )
...
def test_remover_candidato_preserva_a_ordem_dos_demais():
    ...
    classificada = classificar_escola(
        escola, [cand("baixo", {}, longe), cand("medio", {"x": 1}, longe), cand("alto", {"x": 1})]
    )
...
def test_adicionar_e_remover_sao_inversos():
    ...
    classificada = classificar_escola(escola, [cand("a", {"x": 1}), cand("b", {})])
